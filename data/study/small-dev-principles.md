...
더 나아갈 수도 있다. 스타일링 컴포넌트를 웬만하면 따로 만들지 않는 것이다. 컴포넌트의 최상위에 `Container`, `Body` 단 하나의 스타일링 컴포넌트만 두고 후손 셀렉터를 사용해서 나머지를 스타일링하는 것이다. 다음과 같이 말이다.
...
`Body`의 후손 셀렉터를 사용함으로서 스타일링만을 위한 컴포넌트는 `div`나 `p`와 같은 HTML 태그가 그대로 드러나게 하여 "이건 따로 로직을 내장한 컴포넌트가 아니고 오로지 스타일링만을 위해 쓰이는 요소임"을 보여주었다.
...
물론 성능 저하를 자연스럽게 예상할 수 있다. 하지만 (애니메이션도 아니고 단순 스타일링을 위한) CSS까지 쥐어짤 정도의 성능 최적화가 필요한 페이지라면 애초에 css-in-js에서 다른 라이브러리로 넘어가는 게 맞다고 생각한다.
